...
void LBJTab::create_subwindow_from_dialog() {
    ...
    QString channel_name = addDialog->get_entry_list()[2].input_value->text();
    QString pchannel = addDialog->get_entry_list()[3].input_value->text();
    QString nchannel = addDialog->get_entry_list()[4].input_value->text();

    subWindow_list.push_back(new LBJW(new Labjack(runManager, name, type, channel_name, pchannel, nchannel), eventManager));
